/**
 * Fund Source Trace Result Component (Story 5.1)
 *
 * 자금 출처 추적 결과를 표시하는 컴포넌트
 *
 * 기능:
 * - 추적된 거래 체인 목록 렌더링
...
import { ArrowUpCircle, ChevronRight, AlertCircle, Save } from "lucide-react";
...
interface FundSourceTraceResultProps {
...
/**
 * FundSourceTraceResult 컴포넌트
 */
export default function FundSourceTraceResult({
  result,
  isLoading,
  error,
  depth,
  onContinueTracing,
}: FundSourceTraceResultProps) {
    ...
    return (
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            해당 거래와 일치하거나 유사한 출금 거래를 찾을 수 없습니다
          </CardDescription>
        </CardHeader>
      </Card>
    );
        ...
        <div className="rounded-lg border p-4 bg-muted/50">
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-sm font-medium">
                {startTransaction.depositAmount
                  ? formatAmount(Number(startTransaction.depositAmount))
                  : "-"}
              </span>
            </div>
            <div className="flex justify-between">
