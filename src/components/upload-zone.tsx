...
export function FileUploadZone({ caseId, onFilesSelected, onUploadSuccess }: FileUploadProps) {
  ...
  // 템플릿 매칭 확인 핸들러 (맞음, 진행)
  const handleConfirmMatch = useCallback(async (templateId: string | null) => {
    ...
    try {
      ...
      // 데이터 추출
      const extractResult = await extractDataMutation.mutateAsync({ documentId: pendingDocumentId });
      ...
      toast.success(`${extractResult.extractedCount ?? 0}건의 거래가 저장되었습니다`);
