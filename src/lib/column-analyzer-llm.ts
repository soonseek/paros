...
- "거래 후 잔액", "거래후잔액", "잔 액 후송" 등은 balance로 매핑
- "거래금액", "금액" 등 단일 컬럼이면 amount로 매핑

## 비고(memo) 컬럼 인식 - 매우 중요!
다음 컬럼명 또는 유사한 이름은 모두 비고(memo)로 매핑해야 합니다:
- "비고", "적요", "내용", "거래내용", "메모", "설명"
- "계좌 정보", "계좌정보", "결제 정보", "결제정보"
- "계좌 정보/ 결제 정보", "계좌정보/결제정보"
- "상대방", "거래처", "거래상대", "받는분", "보내는분"
- "은행/계좌", "은행", "계좌번호" (텍스트 설명이 포함된 경우)
- 열 이름에 "내역", "정보", "내용"이 포함되면 memo일 가능성 높음

예: "계좌 정보/ 결제 정보" → memo로 매핑!
예: "은행" 컬럼에 "OO은행 홍길동" 형태의 텍스트가 있으면 → memo로 매핑!

## 특수 케이스 1 - 부산은행 등: 비고가 금액 컬럼에 섞여있는 경우
일부 은행에서는 입금/출금 컬럼에 금액 대신 비고(적요)가 들어가는 경우가 있습니다:
- 입금 거래: 출금금액 컬럼에 비고가 들어감 (입금금액에는 숫자)
- 출금 거래: 입금금액 컬럼에 비고가 들어감 (출금금액에는 숫자)

샘플 데이터를 꼼꼼히 확인하세요:
- "맡기신금액" 또는 "찾으신금액" 컬럼에 숫자가 아닌 한글 텍스트(예: "홍길동", "OO상사")가 있으면
- 이것은 memoInAmountColumn: true 케이스입니다!
- 이 경우 별도의 memo 컬럼은 null로 설정하세요.

## 특수 케이스 2 - 비고 컬럼을 찾지 못한 경우
모든 컬럼을 분석했는데도 비고 컬럼을 찾지 못했다면:
1. 금액/잔액/날짜 컬럼이 아닌 나머지 컬럼 중 텍스트가 들어있는 컬럼이 비고일 가능성이 높습니다
2. 샘플 데이터에서 한글 텍스트(상호명, 이름 등)가 들어있는 컬럼을 찾으세요
3. reasoning에 비고 컬럼을 찾지 못한 이유를 상세히 설명하세요
