...
## 사용자 검증 대기 항목
- **P0: OCR 미리보기-최종저장 불일치 수정** - `findColumnIndex` 빈 헤더 버그 + `analyzeWithTemplate` 숫자 매핑 직접 저장 + `performExtraction` 숫자/문자열 이중 처리 (2026-03-10 수정, 사용자 검증 필요)
- 금액 필터 모달 (입금/출금 분리 표시) - 사용자 검증 필요
- 거래 순서 뒤바뀜 수정 - 배포 후 검증 필요
- 보정권고 안내사항 전체 워크플로우 - 배포 후 검증 필요
- 대출 사용 추적 워크플로우 - 사용자 검증 필요
