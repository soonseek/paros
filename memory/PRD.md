...
- [x] **거래내역서 템플릿 관리 시스템 구축 (2026-01-23)**
  - DB 스키마: `TransactionTemplate` 모델 추가
  - 3단계 분류 파이프라인: Layer 1 (정확 매칭) → Layer 2 (LLM 유사도) → Layer 3 (LLM 폴백)
  - 템플릿 에디터 UI (`/admin/templates`)
  - 입금/출금에 따른 컬럼 역할 변경 지원 (`whenDeposit`, `whenWithdrawal`)
  - 템플릿 API (CRUD, 복제, 매칭 테스트)
  - 관리자 설정 페이지에서 템플릿 관리 바로가기 추가

### P1 (진행중)
- [x] 거래 테이블 색상 구분 (입금 파란색/출금 빨간색) - 이미 구현됨
- [x] 거래내역 + AI 어시스턴트 2열 배치 (좌측 60%, 우측 40%)
- [x] 비고(memo) 컬럼 자동 탐지 fallback 로직 추가
  - LLM이 memo를 찾지 못한 경우 헤더에서 키워드 기반 탐지
  - 그래도 못 찾으면 날짜/금액/잔액 외 마지막 컬럼 선택
- [x] 부산은행 특수 케이스 LLM 프롬프트 개선
  - "맡기신금액", "찾으신금액" 컬럼의 비고 혼합 패턴 인식
